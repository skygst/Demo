 ...
 *
 */
public class FragmentGame9 extends Fragment implements OnTouchListener,
		OnClickListener {

	private Context mContext;
	private View view;
	private RelativeLayout rlLayout;
	private ImageView ivCard1, ivCard2, ivCard3, ivCard4, ivTitle1, ivTitle2, ivTitle3, ivTitle4,
			ivBottomBg1, ivBottomBg2, ivBottomBg3, ivBottomBg4, ivBottom1, ivBottom2, ivBottom3, ivBottom4;
	private float scaleQPW = 1.0f, scaleQPH = 1.0f;
	private VideoBiz biz;
	private String[] cardName = {"one", "two", "three", "four"};
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.af_game_9, container, false);
		init();
		setView();
		return view;
	}
	...
	private void init() {
		...
		playSoundMusic(BaseCommon.path_game + BaseCommon.mp3Path(mContext, "pond_animals_prologue.mp3"));
	}
	...
	private void setView() {
		...
		ivTitle1 = (ImageView) view.findViewById(R.id.iv_title_1);
		ivTitle2 = (ImageView) view.findViewById(R.id.iv_title_2);
		ivTitle3 = (ImageView) view.findViewById(R.id.iv_title_3);
		ivTitle4 = (ImageView) view.findViewById(R.id.iv_title_4);
		...
		rlLayout.setBackground(BaseCommon.drawableChange(path, "pond_animals_bg"));
		showCard();
	}
	
	private void showCard() {
		imgTitle = new ImageView[4];
	...
	private void initCard() {
		...
		for(int i=0; i<imgTitle.length; i++) {
			imgTitle[i].setImageDrawable(BaseCommon.drawableChange(path, "pond_animals_title_bg"));
			bottomWord[i].setImageDrawable(BaseCommon.drawableChange(path, "pond_animals_" + cardName[randB[i]]));
			bottomWord[i].setEnabled(true);
			bottomWord[i].setOnTouchListener(this);
			bottomBg[i].setImageDrawable(BaseCommon.drawableChange(path, "pond_animals_bottom_bg"));
		}
	}
	...
	private void commonPosition() {
		...
		setViewPosition(ivTitle1, 4);
		setViewPosition(ivTitle2, 5);
		setViewPosition(ivTitle3, 6);
		setViewPosition(ivTitle4, 7);
		
		setViewPosition(ivBottomBg1, 8);
		setViewPosition(ivBottomBg2, 9);
		setViewPosition(ivBottomBg3, 10);
		setViewPosition(ivBottomBg4, 11);
		
		setViewPosition(ivBottom1, 8);
		setViewPosition(ivBottom2, 9);
		setViewPosition(ivBottom3, 10);
		setViewPosition(ivBottom4, 11);
	}

	private void setViewPosition(View iv, int i) {
		biz.setViewPosition(iv, i, FixedPositionAfrica.pond_animals_position,
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			// 计算该视图在全局坐标系中的x，y值，（注意这个值是要从屏幕顶端算起，也就是索包括了通知栏的高度）//获取在当前屏幕内的绝对坐标
				ivTitle1.getLocationOnScreen(location);
				ivTitle1.getGlobalVisibleRect(viewRect); // 获取img3全局坐标系的一个视图区域，
				// 返回一个填充的Rect对象；该Rect是基于总整个屏幕的
			} else if (indexSelect == 1) {
				ivTitle2.getLocationOnScreen(location);
				ivTitle2.getGlobalVisibleRect(viewRect);
			} else if(indexSelect == 2){
				ivTitle3.getLocationOnScreen(location);
				ivTitle3.getGlobalVisibleRect(viewRect);
			} else if (indexSelect == 3) {
				ivTitle4.getLocationOnScreen(location);
				ivTitle4.getGlobalVisibleRect(viewRect);
			}
	...
	private void setBestPosition(View v, float x, float y) {
		...
		if (indexSelect >= 0 && indexSelect < 4) {
			imgTitle[indexSelect].getLocationOnScreen(location);
		}
		if (indexSelect != -1) {
			showGoodWord(v);
		}
	...
	private void statusImg(ImageView ivImg) {
		...
		if(indexSelect != -1) {
			imgTitle[indexSelect].setImageDrawable(BaseCommon.drawableChange(path, "pond_animals_" + cardName[indexSelect]));
		}
		playSoundMusic(BaseCommon.path_game + cardName[indexSelect] + "_ducks" + ".mp3");
	}
