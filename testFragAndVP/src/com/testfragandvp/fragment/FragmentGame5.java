 ...
 *
 */
public class FragmentGame5 extends Fragment implements OnTouchListener,
		OnClickListener {
	...
	private ImageView ivCard1, ivCard2, ivCard3, ivCard4, ivCard5, ivCard6, ivCard7, ivCard8,
			ivBottom1, ivBottom2, ivBottom3, ivBottom4, ivBottom5, ivBottom6, ivBottom7, ivBottom8;
	private float scaleQPW = 1.0f, scaleQPH = 1.0f;
	private VideoBiz biz;
	private String[] cardName = {"blue", "brown", "gold", "green", "orange", "pink", "purple", "red"};
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.af_game_5, container, false);
		init();
		setView();
		return view;
	}
	...
	private void init() {
		...
		// 提示语
		playSoundMusic(BaseCommon.path_game + BaseCommon.mp3Path(mContext, "isee_my_colors_prologue.mp3"));
	}
	...
	private void setView() {
		...
		ivBottom7 = (ImageView) view.findViewById(R.id.iv_bottom_7);
		ivBottom8 = (ImageView) view.findViewById(R.id.iv_bottom_8);
		rlLayout.setBackground(BaseCommon.drawableChange(path, "my_colors_bg"));
		showCard();
		commonPosition();
	}
	...
	private void showCard() {
		...
		bottomWord[6] = ivBottom7;
		bottomWord[7] = ivBottom8;
		for(int i=0; i<imgCard.length; i++) {
			imgCard[i].setImageDrawable(BaseCommon.drawableChange(path, cardName[rand[i]] + "_default"));
			bottomWord[i].setImageDrawable(BaseCommon.drawableChange(path, cardName[randB[i]]));
	...
	private void commonPosition() {
		setViewPosition(ivCard1, 2);
		setViewPosition(ivCard2, 3);
		setViewPosition(ivCard3, 4);
		setViewPosition(ivCard4, 5);
		setViewPosition(ivCard5, 6);
		setViewPosition(ivCard6, 7);
		setViewPosition(ivCard7, 8);
		setViewPosition(ivCard8, 9);
		setViewPosition(ivBottom1, 10);
		setViewPosition(ivBottom2, 11);
		setViewPosition(ivBottom3, 12);
		setViewPosition(ivBottom4, 13);
		setViewPosition(ivBottom5, 14);
		setViewPosition(ivBottom6, 15);
		setViewPosition(ivBottom7, 16);
		setViewPosition(ivBottom8, 17);
	}

	private void setViewPosition(View iv, int i) {
		biz.setViewPosition(iv, i, FixedPositionAfrica.i_see_my_colors_position,
	...
	public boolean onTouch(View v, MotionEvent event) {
		...
		case MotionEvent.ACTION_UP:
			...
			// 计算该视图在全局坐标系中的x，y值，（注意这个值是要从屏幕顶端算起，也就是索包括了通知栏的高度）//获取在当前屏幕内的绝对坐标
				...
				ivCard7.getLocationOnScreen(location);
				ivCard7.getGlobalVisibleRect(viewRect);
			} else if (indexSelect == 7) {
				ivCard8.getLocationOnScreen(location);
				ivCard8.getGlobalVisibleRect(viewRect);
			}
			...
			if ((indexSelect != -1)
				...
				if (v == ivBottom1) {
					...
					setViewPosition(v, 13);
				}  else if (v == ivBottom5) {
					setViewPosition(v, 14);
				} else if (v == ivBottom6) {
					setViewPosition(v, 15);
				}  else if (v == ivBottom7) {
					setViewPosition(v, 16);
				} else if (v == ivBottom8) {
					setViewPosition(v, 17);
				} 
				// 保护的用途
				selectedId = 0;
			}
	...
	private void statusImg(View ivImg) {
		ivImg.clearAnimation();
		ivImg.setVisibility(View.GONE);
		ivImg.setEnabled(false);
		if(indexSelect != -1) {
			imgCard[indexSelect].setImageDrawable(BaseCommon.drawableChange(path, cardName[rand[indexSelect]] + "_sel"));
			playSoundMusic(BaseCommon.path_game + "my_colors_" + cardName[rand[indexSelect]] + ".mp3");
		}
	}
