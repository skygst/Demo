 ...
 *
 */
public class FragmentGame10 extends Fragment implements OnTouchListener,
		OnClickListener {

	private Context mContext;
	private View view;
	private RelativeLayout rlLayout;
	private ImageView ivCard1, ivCard2, ivCard3, ivCard4, ivCard5, 
			ivBottom1, ivBottom2, ivBottom3, ivBottom4, ivBottom5, ivSide, ivUnderlay;
	private float scaleQPW = 1.0f, scaleQPH = 1.0f;
	private VideoBiz biz;
	private String[] cardName = {"muthroom", "rabbit", "deer", "fox", "stream"};
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.af_game_10, container, false);
		init();
		setView();
		return view;
	}
	...
	private void init() {
		...
		playSoundMusic(BaseCommon.path_game + BaseCommon.mp3Path(mContext, "the_forest_prologue.mp3"));
	}
	...
	private void setView() {
		...
		ivSide = (ImageView) view.findViewById(R.id.iv_side);
		ivUnderlay = (ImageView) view.findViewById(R.id.iv_underlay);
		rlLayout.setBackground(BaseCommon.drawableChange(path, "forest_bg_1"));
		showCard();
	}
	
	private void showCard() {
		imgCard = new ImageView[5];
		bottomWord = new ImageView[5];
		imgCard[0] = ivCard1;
		imgCard[1] = ivCard2;
		imgCard[2] = ivCard3;
		imgCard[3] = ivCard4;
	...
	private void initCard() {
		...
		for(int i=0; i<imgCard.length; i++) {
			if(i != 4) {
				imgCard[i].setImageDrawable(BaseCommon.drawableChange(path, "forest_" + cardName[arrayList[i]] + "_del"));
			}
			bottomWord[i].setImageDrawable(BaseCommon.drawableChange(path, "forest_bottom_" + cardName[randB[i]]));
			bottomWord[i].setEnabled(true);
			bottomWord[i].setOnTouchListener(this);
		}
		ivSide.setImageDrawable(BaseCommon.drawableChange(path, "forest_side_del"));
		ivUnderlay.setImageDrawable(BaseCommon.drawableChange(path, "forest_bg_bottom"));
	}
	...
	private void setViewPosition(View iv, int i) {
		biz.setViewPosition(iv, i, FixedPositionAfrica.the_forest_position,
	...
	private void statusImg(ImageView ivImg) {
		...
		if(indexSelect != -1) {
			if(indexSelect != 4) {
				imgCard[indexSelect].setImageDrawable(BaseCommon.drawableChange(path, "forest_" + cardName[indexSelect] + "_sel"));
			} else {
				rlLayout.setBackground(BaseCommon.drawableChange(path, "forest_bg_2"));
				ivSide.setImageDrawable(BaseCommon.drawableChange(path, "forest_side_sel"));
			}
		}
		playSoundMusic(BaseCommon.path_game + "theforest_" + cardName[indexSelect] + ".mp3");
	}
