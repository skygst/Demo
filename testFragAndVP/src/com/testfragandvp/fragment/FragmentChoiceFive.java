...
public class FragmentChoiceFive extends Fragment implements OnClickListener {
	...
	private void setView() {
		...
		String issureContent = MainActivity.issureContent[4];
		tvIssure.setText(issureContent);
		String[] choiceContent = MainActivity.answerContent[4].split(";");
		choiceSoundContent = MainActivity.choiceSoundContent[4].split(";");
	...
	public void onClick(View v) {
		if (v == btnRead) {
			if (choiceSoundContent != null && choiceSoundContent.length > 0) {
				playSoundMusic(BaseCommon.path_game + choiceSoundContent[0]);
			}
		}
	}
	...
	public class AnswerAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if (null == convertView) {
				...
				holder.btnRead = (Button) convertView
			...
			if(MainActivity.index != 17) {
				holder.tvContent.setVisibility(View.VISIBLE);
				holder.ivPic.setVisibility(View.GONE);
				holder.tvContent.setText(content);
				holder.btnRead.setVisibility(View.VISIBLE);
			} else {
				if(!StringBiz.isEmpty(content) && content.equals("pic_")) {
					holder.tvContent.setVisibility(View.GONE);
					holder.ivPic.setVisibility(View.VISIBLE);
					holder.btnRead.setVisibility(View.INVISIBLE);
					if(position == 0) {
						holder.ivPic.setImageResource(R.drawable.pic_dog);
					} else if(position == 1) {
						holder.ivPic.setImageResource(R.drawable.pic_pig);
					}
				}
			}
			holder.btnOptions.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					ActivityBiz.changeChoiceStatus(4, position);
				}
			});
		...
		public void handleImageView(Button btn, int pos) {
			...
			currentBtn = btn;
			// 可能存在内存问题， 不能够及时释放
			FragmentChoiceFive.adapter.notifyDataSetChanged();
		}
