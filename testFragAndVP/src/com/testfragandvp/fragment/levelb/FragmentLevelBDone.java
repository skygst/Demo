...
public class FragmentLevelBDone extends Fragment implements OnClickListener {
	...
	public void setUserVisibleHint(boolean isVisibleToUser) {
		...
		if (isVisibleToUser) {
			if (ActivityBiz.isFinishAllLevelB()) { // 全部完成， 显示答案对错
				int answerNum = 0;
				LevelBActivity.isShowAnswer = true;
				for (int i = 0; i < LevelBActivity.btnViews.length; i++) {
					if (i > 0 && i < 6) {
						if (ActivityBiz.arrayValue(
								LevelBActivity.choiceAnswerStatus,
								LevelBActivity.replyChoiceAnswerStatus, (i - 1))) {
							answerNum++;
							LevelBActivity.btnViews[i]
