...
/**
 * Three Baby Birds
 *   需求：拖动底图到上面，然后显示句子，并读句子
 * @author
 * 
 */
public class FragmentLevelBGame10 extends Fragment implements OnClickListener, OnTouchListener {

	private Context mContext;
	private View view;
	private RelativeLayout rlLayout;
	private ImageView ivRead, ivCard1, ivCard2, ivCard3, ivCard4, ivCard5;
	private ImageView ivBottomCard1, ivBottomCard2, ivBottomCard3,
			ivBottomCard4, ivBottomCard5, ivSentence;
	private float scaleQPW = 1.0f, scaleQPH = 1.0f;
	private VideoBiz biz;
	private String[] nameId = { "three", "two", "one", "zero", "mama" };
	private String[] sentenceId = { "three", "one", "one", "last_bird", "mama_rest" };
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		init();
		view = inflater.inflate(R.layout.level_b_game_10, container, false);
		setView();
		return view;
	}
	...
	private void init() {
		...
		// 提示语
		playSoundMusic(BaseCommon.pathLevelBGame +  "three_baby_birds.mp3");
	}
	...
	private void commonPosition() {
		...
		for(int i=0; i<ivCard.length; i++) {
			...
			ivCard[i].setImageDrawable(BaseCommon.drawableChange(path, "baby_birds_card_" + nameId[i] + "_bg"));
		}
		...
		for(int i=0; i<ivBottomCard.length; i++) {
			...
			ivBottomCard[i].setImageDrawable(BaseCommon.drawableChange(path, "baby_birds_bottom_" + nameId[rand[i]]));
		}
	...
	private void setViewPosition(View iv, int i) {
		biz.setViewPosition(iv, i, FixedLevelBPosition.lebel_b_10_position,
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				index = selPos;
				iv2.setImageDrawable(BaseCommon.drawableChange(path, "baby_birds_card_" + nameId[selPos]));
				ivSentence.setImageDrawable(BaseCommon.drawableChange(path, "baby_birds_sentence_" + sentenceId[selPos]));
				playSound(selPos);
				if (selectNum >= 5) {
					// 延迟1s --- 跳转到选择题界面
					delayFinish();
	...
	private void playSound(int pos) {
		playSoundMusic(BaseCommon.pathLevelBGame +  "baby_birds_" + sentenceId[pos] + ".mp3");
	}
