...
public class FragmentLevelBChoiceFive extends Fragment implements OnClickListener {
	...
	private void setView() {
		...
		String issureContent = LevelBActivity.issureContent[4];
		tvIssure.setText(issureContent);
		String[] choiceContent = LevelBActivity.answerContent[4].split(";");
		choiceSoundContent = LevelBActivity.choiceSoundContent[4].split(";");
	...
	public class AnswerAdapter extends BaseAdapter {
		...
		public View getView(final int position, View convertView,
			...
			if(LevelBActivity.index != 7) {
				...
				if(!StringBiz.isEmpty(content) && content.equals("pic_")) {
					...
					if(position == 0) { 
						holder.ivPic.setImageResource(R.drawable.pic_goate);
					} else if(position == 1) {
						holder.ivPic.setImageResource(R.drawable.pic_goose);
					}
				}
			}
			holder.btnOptions.setOnClickListener(new OnClickListener() {
				...
				public void onClick(View v) {
					...
					ActivityBiz.changeChoiceLevelBStatus(4, position);
				}
			});
		...
		public void handleImageView(Button btn, int pos) {
			...
			currentBtn = btn;
			// 可能存在内存问题， 不能够及时释放
			FragmentLevelBChoiceFive.adapter.notifyDataSetChanged();
		}
