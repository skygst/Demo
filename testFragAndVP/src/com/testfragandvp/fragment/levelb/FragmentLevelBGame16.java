 ...
 * 
 */
public class FragmentLevelBGame16 extends Fragment implements OnClickListener, OnTouchListener {
	...
	private String[] nameId = { "across", "around", "through", "under" };
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		init();
		view = inflater.inflate(R.layout.level_b_game_16, container, false);
		setView();
		return view;
	}
	...
	private void init() {
		...
		// 提示语
		playSoundMusic(BaseCommon.pathLevelBGame +  "you_can_go.mp3");
	}
	...
	private void refreshData() {
		...
		for(int i=0; i<ivBottomCard.length; i++) {
			ivCard[i].setImageDrawable(BaseCommon.drawableChange(path, "go_card_" + nameId[randB[i]] + "_del"));
			ivBottomBg[i].setImageDrawable(BaseCommon.drawableChange(path, "go_bottom_" + nameId[rand[i]] + "_sel"));
			...
			ivBottomCard[i].setImageDrawable(BaseCommon.drawableChange(path, "go_bottom_" + nameId[rand[i]] + "_del"));
		}
	}
	
	private void setViewPosition(View iv, int i) {
		biz.setViewPosition(iv, i, FixedLevelBPosition.lebel_b_16_position,
	...
	Handler handler = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case 1:
				...
				iv2.setImageDrawable(BaseCommon.drawableChange(path, "go_card_" + nameId[randB[selPos]] + "_sel"));
				ivSentence.setImageDrawable(BaseCommon.drawableChange(path, "go_sentence_" + nameId[randB[selPos]]));
				playSoundMusic(BaseCommon.pathLevelBGame +  "you_can_go_" + nameId[randB[selPos]] + ".mp3");
				if (selectNum >= 4) {
					// 延迟1s --- 跳转到选择题界面
					delayFinish(false);
				}
