 ...
 *
 */
public class FragmentGame14 extends Fragment implements OnTouchListener,
		OnClickListener {

	private Context mContext;
	private View view;
	private RelativeLayout rlLayout, rlBottom;
	private ImageView ivCard1, ivCard2, ivCard3, ivCard4, ivCard5, ivCard6, ivCard7, 
			ivBottom1, ivBottom2, ivBottom3, ivBottom4, ivBottom5, ivBottom6, ivBottom7;
	private float scaleQPW = 1.0f, scaleQPH = 1.0f;
	private VideoBiz biz;
	private String[] cardName = {"one", "two", "three", "four", "five", "six", "seven"};
	...
	public View onCreateView(LayoutInflater inflater, ViewGroup container,
		...
		view = inflater.inflate(R.layout.af_game_14, container, false);
		init();
		setView();
		return view;
	}
	...
	private void init() {
		...
		playSoundMusic(BaseCommon.path_game + BaseCommon.mp3Path(mContext, "counts_kittens_prologue.mp3"));
	}
	...
	private void setView() {
		...
		rlLayout.setBackground(BaseCommon.drawableChange(path, "kitten_bg"));
		showCard();
	}
	
	private void showCard() {
		imgCard = new ImageView[7];
		bottomWord = new ImageView[7];
		imgCard[0] = ivCard1;
		imgCard[1] = ivCard2;
		imgCard[2] = ivCard3;
		imgCard[3] = ivCard4;
	...
	private void initCard() {
		...
		for(int i=0; i<imgCard.length; i++) {
			imgCard[i].setImageDrawable(BaseCommon.drawableChange(path, "kitten_cat_" + (i+1) + "_del"));
			bottomWord[i].setImageDrawable(BaseCommon.drawableChange(path, "kitten_bottom_" + (randB[i] + 1)));
			bottomWord[i].setEnabled(true);
			bottomWord[i].setOnTouchListener(this);
		}
		rlBottom.setBackground(BaseCommon.drawableChange(path, "kitten_bottom_bg"));
	}
	...
	private void commonPosition() {
		...
		setViewPosition(ivBottom7, 13);
		setViewPosition(rlBottom, 14);
	}

	private void setViewPosition(View iv, int i) {
		biz.setViewPosition(iv, i, FixedPositionAfrica.carlos_counts_kittens_position,
	...
	private void statusImg(ImageView ivImg) {
		...
		if(indexSelect != -1) {
			imgCard[indexSelect].setImageDrawable(BaseCommon.drawableChange(path, "kitten_cat_" + (indexSelect + 1) + "_sel"));
		}
		playSoundMusic(BaseCommon.path_game + "carloscountskittens_"+ cardName[selNum] +".mp3");
	}
