...
public class Level8Activity  extends BaseActivity implements OnClickListener {
	/*-----------------基础----------------------*/
	...
	private int iPpCircle[] = {31,46,61,76,85};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.level_08);
	...
	private void initializationData() {		
		mContext = Level8Activity.this;
		scaleQPW = (width/1920.0f);
		scaleQPH = (height/1080.0f);
		
		biz = new VideoBiz();
		path = Constant.path_raz03 + "level08.mp4/";
		currentIndex = 0;//5
		startTime = Constant.timeLevel08[currentIndex][0];
		endTime = Constant.timeLevel08[currentIndex][1];
		haveClicked = false;
		clickNum = 0;
	}
	/*视图初始化*/
	private void setView() {	
		...
		ib_next=(ImageView) findViewById(R.id.ib_next_08);
...
//	    playMusic(MediaCommon.getLevel08Mp3(0));
		mMediaPlayer = biz.playBgMusic(mMediaPlayer, Constant.path_raz03 + "bg_music_level8.mp3");
		/*----------泡泡专用  不共用----------*/
		setPaopaoView();	
		videoCycle(clickNum);
		biz.playVideo(video, path);		
//		CommonUtil.downLoadRes(mContext, 2, 3, Constant.download_level_09, commonPath, "raz03");
	}
	...
	private void setPaopao(){
		...
		iv1_11.setImageDrawable(BaseCommon.drawableChange(Constant.path_raz03_images, "ic_d_1"));
		iv1_21.setImageDrawable(BaseCommon.drawableChange(Constant.path_raz03_images, "ic_d_2"));
		iv1_31.setImageDrawable(BaseCommon.drawableChange(Constant.path_raz03_images, "ic_d_3"));
		iv1_41.setImageDrawable(BaseCommon.drawableChange(Constant.path_raz03_images, "ic_d_4"));
	}
	...
	private void setCommonPosition(ImageView iv,int i){
		iv.setLayoutParams(LayoutParameters.setViewPositionParams(FixedPosition.paopao_position_08[i][0],FixedPosition.paopao_position_08[i][1], FixedPosition.paopao_position_08[i][2], FixedPosition.paopao_position_08[i][3], scaleQPW ,scaleQPH,xoffset,yoffset,suoxiao));						
	}
	...
	private void showBtnafterAndShouByCurrentIndex(){
		...
		/*-------此处的 数字 不共用 需要根据实际修改-------*/
		if(currentIndex == 85 || currentIndex == iPpCircle[4]||currentIndex==117){
	...
	private void setViewPosition(ImageView iv,int i){
		iv.setLayoutParams(LayoutParameters.setViewPositionParams(FixedPosition.level08_position[i][0],FixedPosition.level08_position[i][1], FixedPosition.level08_position[i][2], FixedPosition.level08_position[i][3], scaleQPW ,scaleQPH,xoffset,yoffset,suoxiao));						
	}
	/*显示手动画*/
	private void showBtnAndShou(){
		int i = FixedPosition.getPositionIndex(FixedPosition.level08_index_to_position, currentIndex);
		if(i != -1){
			btn_shou.setVisibility(View.VISIBLE);
			setViewPosition(btn_shou,i);
			CommonAnimation.startShouAnimation(btn_shou);
...
//	/*下一步
	private void showNext(){
		if(currentIndex != 17) return;		
		haveClicked = false;		
		ib_next.setVisibility(View.VISIBLE);
		VideoBiz.startAfterAnimation(ib_next);
		setViewPosition(ib_next,2);
...
//		setStartVr();
		...
		do {
			...
			try {
				...
				if (Constant.timeLevel08[currentIndex][4] == 1  && isClicked) {// && currentIndex != clickIndex
					...
					if(!have4Treasure && (currentIndex == iPpCircle[0] || currentIndex == iPpCircle[1] || currentIndex == iPpCircle[2]||currentIndex == iPpCircle[3])){
						...
						int i = currentIndex;
						if(currentIndex==9){
							playMusicOnly(MediaCommon.getLevel08Mp3(1));
						}
		...
		try {
			...
			if(tem_num == clickNum){
				...
				if(Constant.timeLevel08[currentIndex][4] == 0){
					currentIndex = Constant.timeLevel08[currentIndex][3];
				}
				...
				startTime = Constant.timeLevel08[currentIndex][0] - offset_time;
				endTime = Constant.timeLevel08[currentIndex][1] - offset_time;
	...
	private void playClickMusic(){		
		int i = MusicPosition.getClickMusicPositionIndex(MusicPosition.level08_click_music_position, currentIndex);
		if(i != -1){
			playMusicOnly(MediaCommon.getLevel08Mp3(i));
		}
	}
	/*播放currentIndex对应的声音*/
	private void playCurrentIndexMusic(){		
		int i = MusicPosition.getClickMusicPositionIndex(MusicPosition.level08_music_position, currentIndex);
		if(i != -1){
			Log.d("TESTQQ", "currentIndex对应的声音:"+currentIndex);
			playMusicOnly(MediaCommon.getLevel08Mp3(i));
		}
	...
	private void playReadAfterme(){		
		int i = MusicPosition.getReadAftermeMusicPositionIndex(MusicPosition.level08_readafterme, currentIndex);
		if(i != -1){
	...
	private void playClickHere(int index){		
		...
		int i = MusicPosition.getReadAftermeMusicPositionIndex(MusicPosition.level08_clickhere, index);
		if(i != -1){			
	...
	private void setStartVr(){
		if(currentIndex == 113 || currentIndex == 114 || currentIndex == 115 || currentIndex == 116){
//			handler.sendMessage(handler.obtainMessage(3));
//			sayit3s();
		}
	}
...
//		startActivity(new Intent(mContext, SelectLevelActivity.class));
	...
	private void clickExecute(int seekTo) {
		try{
			/* 不共用  */
			if(currentIndex==17){
				VideoBiz.stopAfterAnimation(ib_next);
				ib_next.setVisibility(View.GONE);
			}
			...
			isCycle = false;
			clickNum++;
			startTime = Constant.timeLevel08[seekTo][0] - offset_time;
			endTime = Constant.timeLevel08[seekTo][1] - offset_time;
			
			video.seekTo(Constant.timeLevel08[seekTo][0] - offset_time);
			isClicked = true;
			currentIndex = seekTo;
...
//		startActivity(new Intent(mContext, SuccessActivity.class).putExtra("level", 10));
		clearMediaVideo();
		finish();
	}
	@Override
	public void onClick(View v) {
		...
		if(v==bt_after){
		  if(currentIndex>MusicPosition.jingyinIndex108&&currentIndex<=MusicPosition.jingyinIndex208){
			//跳转到MV
			  gotoSuccess();
		  }else{
			  gotoSuccess();
		...
		if(v == btn_shou){
			...
			if(shouIsPaopao && currentIndex == iPpCircle[4]){
			...
			}else if(!shouIsPaopao&&clickPaoPao>=3&&currentIndex<=MusicPosition.jingyinIndex108){
				gotoSuccess();
				/*MediaCommon.playSayit(mContext);
				setQiPaoVisible(false);
				clickExecute(87);
				...
				stopBtnAfter();*/
			}else if(currentIndex>MusicPosition.jingyinIndex108&&currentIndex<=MusicPosition.jingyinIndex208){
				//跳转到MV
				gotoSuccess();
			}else{
				clickExecute(Constant.timeLevel08[currentIndex][3]);
	...
	protected void onResume() {
		...
		if(null != mMediaPlayer && !mMediaPlayer.isPlaying() && !(currentIndex >MusicPosition.jingyinIndex108 && currentIndex<MusicPosition.jingyinIndex208)){
			mMediaPlayer.start();
		}
	}
