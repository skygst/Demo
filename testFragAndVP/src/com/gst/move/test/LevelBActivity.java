...
import com.testfragandvp.fragment.levelb.FragmentLevelBChoiceFive;
import com.testfragandvp.fragment.levelb.FragmentLevelBChoiceFour;
import com.testfragandvp.fragment.levelb.FragmentLevelBChoiceOne;
import com.testfragandvp.fragment.levelb.FragmentLevelBChoiceThree;
import com.testfragandvp.fragment.levelb.FragmentLevelBChoiceTwo;
import com.testfragandvp.fragment.levelb.FragmentLevelBDone;

public class LevelBActivity extends FragmentActivity implements OnClickListener {
...
//	public static boolean isFinish = false;
	...
	// 缓存选择题完成的题号
	public static Boolean[] blStatus;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		initViewPage();
		commonPosition();
		issureContent = MultipleChoiceContent.issueLevelBContent[index];
		answerContent = MultipleChoiceContent.answerLevelBContent[index];
		choiceSoundContent = MultipleChoiceContent.choiceLevelBSound[index];
		choiceAnswerStatus = MultipleChoiceContent.choiceLevelBAnswer[index];
	}

	private void init() {
		mContext = LevelBActivity.this;
		isFirstEnter = true;
		isShowAnswer = false;
		blStatus = new Boolean[] {false, false, false, false, false};
...
//		index = getIntent().getExtras().getInt("index_value");
	...
	private void setView() {
		...
		tvTitle.setText(MultipleChoiceContent.levelBTitles[index]);
	...
	private void initViewPage() {
		...
		Fragment fragment = MultipleChoiceContent.fragmentLevelBGame[index];
		fragments.add(fragment); // 游戏
		fragments.add(new FragmentLevelBChoiceOne()); // 选择题 -- 1
		fragments.add(new FragmentLevelBChoiceTwo()); // 选择题 -- 2
		fragments.add(new FragmentLevelBChoiceThree()); // 选择题 -- 3
		fragments.add(new FragmentLevelBChoiceFour()); // 选择题 -- 4
		fragments.add(new FragmentLevelBChoiceFive()); // 选择题 -- 5
		fragments.add(new FragmentLevelBDone()); // 完成题目 -- 2
	...
	private void resetView() {
		for (int i = 0; i < 7; i++) {
			...
			if(i == 6 && ActivityBiz.isFinishAllLevelB()) {
				btnViews[i].setBackgroundResource(finishPicId[i]);
			}
		}
		if(viewPage.result) {
