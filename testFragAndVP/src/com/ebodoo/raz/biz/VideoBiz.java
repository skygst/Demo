...
public class VideoBiz {
	...
	public MyVideoView prepareVideo(MyVideoView video, String path) {
		try {
	...
	public MediaPlayer playBgMusic(MediaPlayer mMediaPlayer, String path) {
		try {
	...
	public void setViewPosition(View view, int i, int level_position[][], float scaleQPW, float scaleQPH, int xoffset, int yoffset, float suoxiao) {
		view.setLayoutParams(LayoutParameters.setViewPositionParams(
				level_position[i][0], // 图片宽
				level_position[i][1],// 图片高
				level_position[i][2], // 在布局里的x轴位置
				level_position[i][3], // 在布局里的y轴位置
				scaleQPW, scaleQPH,
				xoffset, yoffset, suoxiao));
	}
	
	
	public void setScorePosition(View view,int x, int y, float scaleQPW, float scaleQPH){
		view.setLayoutParams(LayoutParameters.setViewPositionParams(
				82,
				34,
				x,
...
//	public Object[] buyVipView(Context context, boolean isEg) {
//		String title = "", cancel = "", buy = "";
//		if(isEg) {
//			title = context.getString(R.string.eg_buy_vip_title);
//			cancel = context.getString(R.string.eg_quxiao);
//			buy = context.getString(R.string.eg_purchase);
//		} else {
//			title = context.getString(R.string.ch_buy_vip_title);
//			cancel = context.getString(R.string.ch_quxiao);
//			buy = context.getString(R.string.ch_purchase);
//		}
...
//	}
	...
	public String delayReadWordLevel07(boolean isEg, int num) {
		String path = "";
		if (num == 17 || num == 18) {
			path = MediaCommon.getLevel07Mp3(isEg, 2);
		} else if (num == 19 || num == 20) {
			path = MediaCommon.getLevel07Mp3(isEg, 3);
		} else if (num == 21 || num == 22) {
			path = MediaCommon.getLevel07Mp3(isEg, 4);
		} else if (num == 23 || num == 24) {
			path = MediaCommon.getLevel07Mp3(isEg, 5);
		}
		return path;
	}
	
	public String readWithMeLevel07(boolean isEg, int index) {
		String path = "";
		if (index == 0) {
			path = MediaCommon.getLevel07Mp3(isEg, 8);
		} else if (index == 1) {
			path = MediaCommon.getLevel07Mp3(isEg, 9);
		} else if (index == 2) {
			path = MediaCommon.getLevel07Mp3(isEg, 10);
		} else if (index == 3) {
			path = MediaCommon.getLevel07Mp3(isEg, 11);
		}
		return path;
	}
	
//	public AnimationDrawable ss() {
