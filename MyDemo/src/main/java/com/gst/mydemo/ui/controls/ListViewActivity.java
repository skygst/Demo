...
import com.gst.mydemo.adapter.MyListNewAdapter;
 ...
 */
public class ListViewActivity extends TopActivity implements View.OnClickListener {
    ...
    private MyListNewAdapter adapter2;
    ...
    private void setView() {
        ...
        adapter = new MyListAdapter(mContext, listStu, mIndex);
    ...
    private void listOnListener() {
        listView.setOnItemSelectedListener(new AdapterView.OnItemSelectedListener() {
            ...
            public void onNothingSelected(AdapterView<?> adapterView) {
                System.out.println("pos --------22222--------Selected:");
                adapterView.setDescendantFocusability(ViewGroup.FOCUS_BEFORE_DESCENDANTS);
                ...
                case 0:
                    if(adapter != null) {
                        if(pos++ < 15) {
                            ...
                            adapter = new MyListAdapter(mContext, listStu, mIndex);
                            listView.setAdapter(adapter);
                            listView.setSelection(pos + 100);
                            listView.smoothScrollToPosition(pos + 100);
                            delayAdapter();
                        }
                    }
                    break;
...
//                List<CashRecorde> recordeListValue = (List<CashRecorde>) obj[0];
                if (stuList != null) {
                    ...
                    if (stuList.size() > 0) {
                        ...
                        if (page == 0) {
                                adapter = new MyListAdapter(mContext, recordeList, mIndex);
                                listView.setAdapter(adapter);
                            } else {
                                if (adapter != null)
                                    adapter.notifyDataSetChanged();
